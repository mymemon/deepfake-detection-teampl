...
# ------------------------------
```
# 이미지 변환 구성: 크기 조정 후 텐서 형식으로 변환
transform = transforms.Compose([
    transforms.Resize(image_size),  # 지정된 크기로 이미지 리사이즈
    transforms.ToTensor(),          # 이미지를 텐서 형식으로 변환
])

# 학습 및 검증 데이터 로드: 폴더 구조를 기반으로 라벨 자동 생성
train_data = datasets.ImageFolder(train_folder, transform=transform) # 학습 데이터셋 생성
val_data = datasets.ImageFolder(val_folder, transform=transform)# 검증 데이터셋 생성

# 데이터 로더 생성: 배치 단위로 데이터를 로드하며 학습/검증 데이터 분리
train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True) # 학습용 데이터 로더 (셔플 활성화)
val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False)  # 검증용 데이터 로더 (셔플 비활성화)
...
# ------------------------------
```
# 사전 학습된 ResNet-18 모델을 로드
model = models.resnet18(pretrained=True)  # ResNet-18 모델 불러오기 (사전 학습된 가중치 사용)
model.fc = nn.Linear(model.fc.in_features, len(train_data.classes))  # 출력층을 데이터 클래스 수에 맞게 수정

# 손실 함수 및 최적화 기법 설정
criterion = nn.CrossEntropyLoss()  # 다중 클래스 분류를 위한 교차 엔트로피 손실 함수
optimizer = optim.Adam(model.parameters(), lr=learning_rate)  # Adam 옵티마이저 사용, 학습률 설정
```
# ------------------------------
# 4. 학습 함수
# ------------------------------
...
def train_model(model, train_loader, criterion, optimizer, num_epochs):
    """
    모델 학습 함수: 주어진 모델을 데이터 로더, 손실 함수, 옵티마이저, 학습 에포크 수를 기반으로 학습시킴.
    
    Args:
        model: 학습할 모델 객체
        train_loader: 학습 데이터 로더 (데이터셋 및 배치 제공)
        criterion: 손실 함수
        optimizer: 옵티마이저 (가중치 업데이트를 위한 알고리즘)
        num_epochs: 학습 반복 횟수 (에포크 수)
    """
    model.train()  # 모델을 학습 모드로 설정
    for epoch in range(num_epochs):   # 지정된 에포크 수만큼 학습 반복
        running_loss = 0.0    # 현재 에포크의 누적 손실 초기화
        for images, labels in train_loader:
            optimizer.zero_grad()   # 옵티마이저의 기울기 초기화
            outputs = model(images)  # 모델에 이미지를 입력하여 예측 결과(outputs) 계산
            loss = criterion(outputs, labels)  # 예측 값과 실제 레이블 간의 손실 계산
            loss.backward()  # 손실을 기반으로 역전파 수행하여 기울기 계산
            optimizer.step()  # 옵티마이저가 가중치를 업데이트
            # 배치의 손실을 누적 (손실에 배치 크기를 곱하여 전체 데이터셋의 손실 계산)
            running_loss += loss.item() * images.size(0)

        # 에포크별 평균 손실 계산 (전체 데이터셋에 대한 평균)
...
def validate_model(model, val_loader, criterion):
    """
    모델 검증 함수: 주어진 모델을 검증 데이터셋을 기반으로 평가하며, 손실 및 정확도를 계산.
    
    Args:
        model: 검증할 모델 객체
        val_loader: 검증 데이터 로더 (데이터셋 및 배치 제공)
        criterion: 손실 함수
    """

    model.eval()   # 모델을 평가 모드로 설정
    running_loss = 0.0   검증 데이터셋에서 누적 손실 초기화
    correct = 0  # 올바르게 예측한 샘플 개수 초기화
    with torch.no_grad():  # 검증 시에는 기울기를 계산하지 않음
        for images, labels in val_loader:   # val_loader에서 배치 단위로 데이터를 가져옴
            outputs = model(images)  # 모델에 이미지를 입력하여 예측 결과(outputs) 계산
            loss = criterion(outputs, labels)  # 예측 값과 실제 레이블 간의 손실 계산
            # 배치 손실을 데이터셋 전체 크기에 맞게 누적
            running_loss += loss.item() * images.size(0)
            # 모델의 예측 값 계산 (가장 높은 점수를 가진 클래스 선택)
            _, preds = torch.max(outputs, 1)
            # 예측이 정답과 일치한 샘플 수를 누적
            correct += (preds == labels).sum().item()
    # 검증 데이터셋 전체 손실 평균 계산
    val_loss = running_loss / len(val_loader.dataset)
    # 정확도 계산 (올바르게 예측한 샘플 수를 전체 샘플 수로 나눔)
